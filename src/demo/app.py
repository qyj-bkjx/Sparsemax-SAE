...
def load_results(resized_image, radio_choice, clip_act, maple_act, toggle_btn):
    ...
    print("init_seg:", type(init_seg))
    print("maple_init_seg:", type(maple_init_seg))
    print("init_tops:", init_tops, "number:", len(init_tops))
    # if len(init_tops) < 1:
    #     init_tops = [None]

    print("init_values:", init_values, "number:", len(init_values))

    out = (init_seg, maple_init_seg)
    out += tuple(init_tops)
    out += tuple(init_values)
    print("load_results out number:", len(out), "content:", out)
...
if __name__ == "__main__":
    ...
    sae_tester = load_sae_tester("path to sae model", args.include_imagenet)
    ...
    for dataset in ["imagenet"]:
        data = torch.load(
            f"your max_activating_image_values.pt path",
            map_location="cpu",
        )
