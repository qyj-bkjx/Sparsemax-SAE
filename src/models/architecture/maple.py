...
from src.models.clip.clip import tokenize
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.proj = nn.Linear(ctx_dim, 768).type(dtype)
        # self.proj.half()
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, pixel_values):
        # alhtough input_ids and attention_mask are not used, but they are required for the forward pass **kwargs
        _, shared_ctx, _, deep_compound_prompts_vision = self.prompt_learner()
        image_features = self.image_encoder(
            pixel_values.type(self.dtype), shared_ctx, deep_compound_prompts_vision
        )
