...
class SAETrainer:
    def __init__(
        ...
        self.active_frac_threshold = getattr(cfg, "active_frac_threshold", 0.1)
        self.metrics_csv_path = getattr(
            cfg,
            "metrics_csv_path",
            os.path.join(self.cfg.checkpoint_path, "active_over_threshold.csv"),
        )
        self._metrics_rows = [] 
    @torch.no_grad()
    def _active_count_over_threshold_window(self):


        if self.n_frac_active_tokens == 0:
            return 0, 0.0
        feature_freq = (self.act_freq_scores / self.n_frac_active_tokens).detach()
        count = int((feature_freq > self.active_frac_threshold).sum().item())
        ratio = count / self.cfg.d_sae
        # print("_active_count_over_threshold_window")
        # print(ratio)
        return count, ratio
    ...
    def _flush_metrics_csv(self):
        ...
        fieldnames = list(self._metrics_rows[0].keys())
        with open(tmp_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(self._metrics_rows)
        os.replace(tmp_path, self.metrics_csv_path)
        print(f"Wrote metrics CSV to {self.metrics_csv_path}")
    ...
    def fit(self) -> SparseAutoencoder:
        ...
        try:
            ...
            while self.n_training_tokens < self.cfg.total_training_tokens:
                ...
                # 记录当前 step 的窗口统计与累计统计（累计来自 SAE 内部的 idf_socre）

                count_win, ratio_win = self._active_count_over_threshold_window()
                count_cum = int((self.sae.idf_socre > self.active_frac_threshold).sum().item())
                ratio_cum = count_cum / self.cfg.d_sae

                self._metrics_rows.append({
                    "step": int(self.n_training_steps + 1),
                    "tokens_seen": int(self.n_training_tokens),
                    "threshold": float(self.active_frac_threshold),
                    "count_window": int(count_win),
                    "ratio_window": float(ratio_win),
                    "count_cumulative": int(count_cum),
                    "ratio_cumulative": float(ratio_cum),
                })
