...
def rectangle(x, bandwidth):
    return ((x > -0.5) & (x < 0.5)).float() / bandwidth
...
class StepFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_x = torch.zeros_like(grad_output)  # No grad to x
        grad_threshold = -rectangle((x - threshold) / bandwidth, bandwidth) * grad_output
        return grad_x, grad_threshold.sum(), None  # None for bandwidth

def step(x, threshold, bandwidth=1.0):
...
class JumpReLUFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_threshold = -threshold * rectangle((x - threshold) / bandwidth, bandwidth) * grad_output
        return grad_x, grad_threshold.sum(), None

def jumprelu(x, threshold, bandwidth=1.0):
    return JumpReLUFunction.apply(x, threshold, bandwidth)
def _pow_masked(u, p, mask):
    u_safe = torch.where(mask, u, torch.ones_like(u))
    return (u_safe ** p) * mask
class SparseAutoencoder(HookedRootModule):
    ...
    def __init__(
        ...
        self.log_threshold = nn.Parameter(torch.tensor(0.0, dtype=self.dtype, device=self.device))
        self.bandwidth = 1.0 

        self.topk_k = getattr(cfg, "topk_k", 32)  # default K=20

        self.setup()  # Required for `HookedRootModule`s

        self.concept = nn.Parameter(
                torch.empty(self.d_sae, self.d_in, dtype=self.dtype, device=self.device)
            )
        torch.nn.init.kaiming_uniform_(self.concept.T)
        self.v = nn.Parameter(
            nn.init.kaiming_uniform_(
            torch.empty(self.d_in, self.d_in, dtype=self.dtype, device=self.device)
        )
        )
        ...
        self.count_batch = 0
        self.idf_socre  = torch.tensor(0.0, dtype=self.dtype, device=self.device)
    def forward(self, x, dead_neuron_mask=None):

        return self.forward_standard(x, dead_neuron_mask)

    def sparsemax(self, score, dim=-1):
        """
        Computes the sparsemax of the input tensor along the specified dimension.
        Sparsemax is a differentiable alternative to softmax that produces sparse outputs.
        """
    ...
    def forward_standard(self, x, dead_neuron_mask=None):
        ...
        x_n = x / torch.norm(x, dim=-1, keepdim=True)  # normalize x to have unit norm

        sae_in = self.hook_sae_in(x_n)

        n_dim = len(x.size())


        concept_k = self.concept @ self.k
        ...
        idf_mask = (self.idf_socre > 0.1).float()
        hidden_pre = hidden_pre * (1 - idf_mask)  # apply idf mask to hidden_pre
        sparse_probs = self.sparsemax(hidden_pre)  # apply sparsemax to hidden_pre
        mask = (sparse_probs > 0).float()
        feature_acts = self.hook_hidden_post(hidden_pre * mask)   # apply mask

        self.idf_socre = self.idf_socre * (self.count_batch-1) + (feature_acts.detach().view(-1, self.d_sae) > 0).float().mean(0)
        self.idf_socre /= self.count_batch
        self.sparsity_coefficient = 0.0
        
        concept_v = self.concept @ self.v
        x_rec = einops.einsum(
                feature_acts,
                concept_v,
        ...
        x_rec_sclae = x_rec*torch.norm(x, dim=-1, keepdim=True)

        sae_out = self.hook_sae_out(x_rec_sclae)  # add back the mean and std to the output
        mse_loss = (
            torch.pow((x_rec_sclae - x), 2)
            / (x0**2).sum(dim=-1, keepdim=True).sqrt()
        )
        ...
        # print(f"mse_loss_ghost_resid mean: {mse_loss_ghost_resid.mean().item():.6f}")

        if len(mse_loss.size()) == 3 and self.training:
            mse_loss[:, 0, :] = mse_loss[:, 0, :] * self.cfg.mse_cls_coefficient

        mse_loss = mse_loss.mean() * 10
        sparsity = torch.abs(feature_acts).sum(dim=-1).mean(dim=(0,))
        # sparsity = - (feature_acts * torch.log(feature_acts + 1e-10)).sum(-1).mean(dim=(0,))
        l1_loss = self.l1_coefficient * sparsity * self.sparsity_coefficient
        
        loss = mse_loss + mse_loss_ghost_resid + l1_loss
    ...
    def forward_clamp(
        ...
        clamp_feat_dim=None,   # bool mask / indices / None
        clamp_value=1.0        # scalar or vector
    ):
        ...
        hidden_pre = self.hook_hidden_pre(
            einops.einsum(
                sae_in, concept_k.T, "... d_in, d_in d_sae -> ... d_sae"
            )
        )

        sparse_probs = self.sparsemax(hidden_pre)
        acts = hidden_pre * (sparse_probs > 0).float()


        feature_acts = self.hook_hidden_post(acts)

        d_sae = self.d_sae
        device = feature_acts.device
        dtype = feature_acts.dtype

        scale_vec = torch.ones(d_sae, dtype=dtype, device=device)

        if clamp_feat_dim is None:

            if torch.is_tensor(clamp_value):
                clamp_value = clamp_value.to(device=device, dtype=dtype)
                if clamp_value.ndim == 0:
                    scale_vec = scale_vec * clamp_value
                else:
                    assert clamp_value.numel() == d_sae, \
                        f"clamp_value length {clamp_value.numel()} must equal d_sae={d_sae} when clamp_feat_dim is None"
                    scale_vec = clamp_value.view(-1)
            else:
                # python 标量
                scale_vec = scale_vec * float(clamp_value)
        else:

            if isinstance(clamp_feat_dim, torch.Tensor) and clamp_feat_dim.dtype == torch.bool:
                sel_mask = clamp_feat_dim.to(device=device)
                sel_idx = torch.nonzero(sel_mask, as_tuple=False).view(-1)
            else:
                sel_idx = torch.as_tensor(clamp_feat_dim, device=device).long()

            if torch.is_tensor(clamp_value):
                clamp_value = clamp_value.to(device=device, dtype=dtype)
                if clamp_value.ndim == 0:
                    scale_vec[sel_idx] = clamp_value
                else:
                    assert clamp_value.numel() == sel_idx.numel(), \
                        f"clamp_value length {clamp_value.numel()} must equal number of selected features {sel_idx.numel()}"
                    scale_vec[sel_idx] = clamp_value.view(-1)
            else:
                scale_vec[sel_idx] = float(clamp_value)


        view_shape = [1] * (feature_acts.ndim - 1) + [-1]
        feature_acts = feature_acts * scale_vec.view(*view_shape)

        concept_v = self.concept @ self.v
        x_rec = einops.einsum(
            feature_acts, concept_v, "... d_sae, d_sae d_in -> ... d_in"
        )
        x_rec_scale = x_rec * torch.norm(x, dim=-1, keepdim=True).clamp_min(1e-6)
        sae_out = self.hook_sae_out(x_rec_scale)

        mse = ( (x_rec_scale - x) ** 2 ) / (x**2).sum(dim=-1, keepdim=True).sqrt().clamp_min(1e-6)
        if mse.ndim == 3 and self.training:
            mse[:, 0, :] = mse[:, 0, :] * getattr(self.cfg, "mse_cls_coefficient", 1.0)
        mse_loss = mse.mean() * 10

        l1_loss = torch.tensor(0.0, dtype=dtype, device=device)
        mse_loss_ghost_resid = torch.tensor(0.0, dtype=dtype, device=device)
        loss = mse_loss + l1_loss + mse_loss_ghost_resid

        return sae_out, feature_acts, loss, mse_loss, l1_loss, mse_loss_ghost_resid 
    
    def count_active_neurons(self, frac_threshold: float = 0.1) -> int:
        return int((self.idf_socre > frac_threshold).sum().item())

    def frac_active_neurons(self, frac_threshold: float = 0.1) -> float:
        return float((self.idf_socre > frac_threshold).float().mean().item())
