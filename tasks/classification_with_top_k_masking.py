...
def dump_activation_counts(cls_sae_cnt: np.ndarray, classnames: list, save_directory: str):

    os.makedirs(save_directory, exist_ok=True)
    num_classes, sae_dim = cls_sae_cnt.shape

    global_sum = cls_sae_cnt.sum(axis=0)         
    global_mean = cls_sae_cnt.mean(axis=0)        
    global_rank_sum = np.argsort(-global_sum)   
    global_rank_mean = np.argsort(-global_mean)  
    df_sum = pd.DataFrame({
        "feature_idx": np.arange(sae_dim),
        "count_sum": global_sum,
        "rank_by_sum": np.empty(sae_dim, dtype=int)
    })
    rank_by_sum = np.empty(sae_dim, dtype=int)
    rank_by_sum[global_rank_sum] = np.arange(1, sae_dim + 1)
    df_sum["rank_by_sum"] = rank_by_sum
    df_sum = df_sum.sort_values("rank_by_sum")
    df_sum.to_csv(os.path.join(save_directory, "global_activation_sum.csv"), index=False)

    df_mean = pd.DataFrame({
        "feature_idx": np.arange(sae_dim),
        "count_mean": global_mean,
        "rank_by_mean": np.empty(sae_dim, dtype=int)
    })
    rank_by_mean = np.empty(sae_dim, dtype=int)
    rank_by_mean[global_rank_mean] = np.arange(1, sae_dim + 1)
    df_mean["rank_by_mean"] = rank_by_mean
    df_mean = df_mean.sort_values("rank_by_mean")
    df_mean.to_csv(os.path.join(save_directory, "global_activation_mean.csv"), index=False)

    for class_idx, classname in enumerate(classnames):
        counts = cls_sae_cnt[class_idx]           
        order = np.argsort(-counts)               
        ranks = np.empty_like(order)
        ranks[order] = np.arange(1, sae_dim + 1)

        df_cls = pd.DataFrame({
            "feature_idx": np.arange(sae_dim),
            "count": counts,
            "rank_in_class": ranks
        }).sort_values("rank_in_class")

        safe_name = str(classname).replace("/", "_")
        df_cls.to_csv(
            os.path.join(save_directory, f"class_{class_idx}_{safe_name}_activation_counts.csv"),
            index=False
        )
...
def main(
    ...
    root_dir: str,
    save_name: str,
    backbone: str = "your backbone path",
    batch_size: int = 8,
    model_path: str = None,
    ...
    for class_idx, classname in enumerate(tqdm(classnames)):
        ...
        for k, v in preds_dict.items():
            acc = v.count(class_idx) / len(v) * 100
            metrics_dict[class_idx][k] = acc
            print(f"[Class {class_idx} - {classname}] {k}: {acc:.2f}%")

    metrics_df = pd.DataFrame(metrics_dict)
    metrics_df.to_csv(f"{save_directory}/metrics.csv", index=False)
    print(f"metrics.csv saved at {save_directory}")
    avg_series = metrics_df.mean(axis=1) 
    avg_df = avg_series.reset_index()
    avg_df.columns = ["setting", "mean_acc"] 

    avg_df.to_csv(f"{save_directory}/metrics_mean.csv", index=False)
    print(f"metrics_mean.csv saved at {save_directory}")
