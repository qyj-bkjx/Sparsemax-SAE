...
def get_sae_activations(
    ...
    if len(activations.shape) == 3:  # (batch, seq_len, features)
        feature_counts = activations.sum(dim=0).sum(dim=0)
    elif len(activations.shape) == 2:  # (batch, features)
        feature_counts = activations.sum(dim=0)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="outoursae",
        help="save name for feature data",
    )
